  ...
  protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
    ...
    if (!request.getRequestURI().contains("private")) {
      ...
      try {
        ...
        UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(UserSecurity.fromTokenClaim(claims), null, List.of(new SimpleGrantedAuthority(claims.get("role", String.class))));
        SecurityContextHolder.getContext().setAuthentication(auth);
        log.info("JWT 认证成功: 用户名={}, 角色={}", claims.getSubject(), claims.get("role", String.class));
